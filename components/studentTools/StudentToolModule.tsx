...
import type { StudentToolDefinition } from "@/lib/studentTools";
import { getStudentToolConfig } from "@/lib/studentToolConfigs";
import type { TeacherToolField } from "@/lib/teacherToolConfigs";

interface StudentToolModuleProps {
  tool: StudentToolDefinition;
}
...
export function StudentToolModule({ tool }: StudentToolModuleProps) {
  const config = getStudentToolConfig(tool.slug);
  ...
  const generate = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        document.getElementById("student-tool-output")?.scrollIntoView({ behavior: "smooth" });
      }, 100);
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="space-y-4">
          {fields.map((field) => (
            <div key={field.id} className="bg-white rounded-2xl border border-neutral-100 p-5 shadow-sm space-y-3">
              <label
                htmlFor={`student-field-${field.id}`}
      ...
      {output && (
        <div id="student-tool-output" className="bg-white rounded-2xl border border-neutral-100 p-5 shadow-sm space-y-4">
